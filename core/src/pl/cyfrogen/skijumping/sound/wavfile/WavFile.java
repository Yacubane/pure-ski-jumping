package pl.cyfrogen.skijumping.sound.wavfile;
...
public class WavFile
{
    ...
    public static WavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
    {
        ...
        putLE(16, wavFile.buffer, 4, 4);        // Chunk SaveData Size
        ...
        // Start SaveData Chunk
        putLE(DATA_CHUNK_ID,				wavFile.buffer, 0, 4);		// Chunk ID
        putLE(dataChunkSize, wavFile.buffer, 4, 4);        // Chunk SaveData Size
    ...
    public static WavFile openWavFile(InputStream file, long length) throws IOException, WavFileException
    {
        ...
        if (length != chunkSize+8) {
            throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + length + ")");
        }

        boolean foundFormat = false;
        boolean foundData = false;

        // Search for the Format and SaveData Chunks
        while (true)
        {
            ...
            else if (chunkID == DATA_CHUNK_ID)
            {
                ...
                if (foundFormat == false)
                    throw new WavFileException("SaveData chunk found before Format chunk");

                // Check that the chunkSize (wav data length) is a multiple of the
                // block align (bytes per frame)
                if (chunkSize % wavFile.blockAlign != 0)
                    throw new WavFileException("SaveData Chunk size is not multiple of Block Align");
