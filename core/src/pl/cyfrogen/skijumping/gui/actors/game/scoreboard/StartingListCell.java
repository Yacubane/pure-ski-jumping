...
public class StartingListCell extends Group {
    public StartingListCell(int hillNum, WorldCupJumperData worldCupJumperData, float width, float height) {
        ...
        if(worldCupJumperData.hasPlace(hillNum)) {
            ...
            FilledLabel positionLabel = new FilledLabel(
                    background.getHeight(),
                    background.getHeight(),
                    0.9f, 0.4f, String.valueOf(worldCupJumperData.getPlace(hillNum)),
                    new Label.LabelStyle(Main.getInstance().getAssets().getFont(), Color.BLACK));
            positionLabel.setX(width * 0.8f);
